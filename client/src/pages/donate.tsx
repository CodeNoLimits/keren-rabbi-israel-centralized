...
export default function Donate() {
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F6] dark:bg-[#050505]" dir={isRTL ? 'rtl' : 'ltr'}>
      <Header currentLanguage={currentLanguage} onLanguageChange={setLanguage} />
      
      {/* Hero Section */}
      <section className="relative overflow-hidden pt-32 pb-20 lg:pt-40 lg:pb-32 bg-white dark:bg-[#0A0A0B] border-b border-slate-200 dark:border-white/5">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-[#B5912B] dark:text-[#D4AF37] font-semibold mb-8 shadow-sm">
              <Sparkles className="w-4 h-4" />
              <span>{t.heroSubtitle}</span>
            </div>
            
            <h1 className={`text-5xl lg:text-7xl font-bold text-slate-900 dark:text-white mb-6 tracking-tight leading-tight ${rtlFont(isRTL)}`} style={{ textShadow: '0 10px 30px rgba(0,0,0,0.1)' }}>
              {t.heroTitle}
            </h1>
            <GoldDivider size="md" className="mb-8" />
            
            <p className="text-xl text-slate-700 dark:text-slate-300 mb-12 max-w-3xl mx-auto leading-relaxed font-light">
              {t.heroDescription}
            </p>
          </motion.div>
        </div>
      ...
      <section className="py-20 relative z-20">
        <div className="container mx-auto px-4">
          ...
          >
            <h2 className={`text-4xl lg:text-5xl font-bold text-slate-900 dark:text-white mb-4 ${rtlFont(isRTL)}`}>
              {t.tiersTitle}
            </h2>
          ...
          {/* Cards Grid */}
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto mb-20 animate-fade-in-up">
            {t.tiers.map((tier, index) => (
              <motion.div
                ...
                className={`relative p-8 rounded-3xl cursor-pointer transition-all duration-500 border-2 overflow-hidden bg-white dark:bg-[#0A0A0B] shadow-[0_8px_30px_rgba(0,0,0,0.06)] hover:shadow-2xl ${
                  selectedTier === tier.id 
                    ? 'border-[#D4AF37] transform -translate-y-2' 
                    : 'border-transparent hover:border-[#D4AF37]/50 dark:hover:border-[#D4AF37]/50 dark:border-white/5 border-slate-100'
                }`}
              >
                {/* Popular Badge */}
                {tier.popular && (
                  <div className="absolute top-0 right-0 bg-gradient-to-r from-[#D4AF37] to-[#B5912B] text-white text-xs font-bold px-4 py-1.5 rounded-bl-xl z-20 uppercase tracking-widest">
                ...
                <div className="relative z-10">
                  <div className="text-5xl mb-6 drop-shadow-sm">{tier.icon}</div>
                  <h3 className={`text-2xl font-bold text-slate-900 dark:text-white mb-2 ${rtlFont(isRTL)}`}>{tier.name}</h3>
                  <div className="flex items-baseline gap-1 mb-6 text-[#B5912B] dark:text-[#D4AF37]">
                    <span className="text-2xl font-medium">$</span>
                    <span className="text-5xl font-extrabold tracking-tight">{tier.price}</span>
                  </div>
                  <p className="text-slate-600 dark:text-slate-400 text-sm mb-8 min-h-[60px] font-light leading-relaxed">
                    {tier.description}
                  </p>
                  ...
                  <div className={`flex items-center justify-center gap-2 p-3.5 rounded-xl border-2 transition-all duration-300 ${
                    selectedTier === tier.id 
                      ? 'bg-gradient-to-r from-[#D4AF37] to-[#B5912B] text-white border-transparent shadow-[0_4px_14px_rgba(212,175,55,0.4)]' 
                      : 'border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-[#1A1C23]'
                  }`}>
                    {selectedTier === tier.id ? (
                    ...
                    <span className="font-bold tracking-wide">{currentLanguage === 'he' ? 'בחר מסלול זה' : 'Select Tier'}</span>
                  </div>
                </div>
              </motion.div>
            ))}
          ...
          <motion.div 
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="max-w-2xl mx-auto bg-white dark:bg-[#1A1C23] p-10 md:p-12 rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.08)] border border-slate-100 dark:border-white/5 relative overflow-hidden"
          >
            <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-[#D4AF37] to-[#B5912B]" />
            <div className="mb-10">
              <label className={`block text-xl font-bold text-slate-900 dark:text-white mb-6 text-center ${rtlFont(isRTL)}`}>
                {t.selectAmount}
              </label>
              <div className="relative max-w-xs mx-auto">
                ...
                <input
                  ...
                  className="w-full pl-14 pr-6 py-4 rounded-xl border-2 border-slate-200 dark:border-white/10 focus:border-[#D4AF37] dark:focus:border-[#D4AF37] bg-slate-50 dark:bg-[#0A0A0B] text-2xl font-bold text-slate-800 dark:text-white outline-none transition-all shadow-inner"
                />
              </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-6">
              <button
                ...
                className={`w-full max-w-md py-5 rounded-xl font-bold text-lg flex items-center justify-center gap-3 transition-all duration-300 ${
                  (!selectedTier && !customAmount)
                    ? 'bg-slate-200 dark:bg-white/5 text-slate-400 dark:text-slate-600 cursor-not-allowed'
                    : 'bg-gradient-to-r from-[#D4AF37] to-[#B5912B] hover:from-[#B5912B] hover:to-[#9E7A1C] text-white shadow-[0_4px_20px_rgba(212,175,55,0.4)] transform hover:-translate-y-1'
                }`}
              >
                {isProcessing ? (
                  <>
              ...
              <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 text-sm font-medium bg-slate-50 dark:bg-[#0A0A0B] px-5 py-2.5 rounded-full border border-slate-200 dark:border-white/5">
